...
if(checkName.includes('fuck') ||checkName.includes('penis') ||checkName.includes('pussy') ||checkName.includes('nigger') ||checkName.includes('faggot') ||checkName.includes('shit') ||checkName.includes('bitch') || checkName.includes('cock')|| checkName.includes('sex') || checkName.includes('dick')){
...
fetch('/updateScore?score='+score).then(function(){getHighScore();})}
...
managePlayer: function(){
  for (index in this.platforms){
if(this.character.x + 5 < this.platforms[index].x + this.platforms[index].w && this.platforms[index].x < this.character.x && this.platforms[index].y > this.character.y + this.character.w ){if(jumping === false){this.character.y += 1.5;}}
if(this.character.y + this.character.w >= this.platforms[index].y && this.platforms[index].x < this.character.x + this.character.w && this.character.x < this.platforms[index].x + this.platforms[index].w){
postJump = false; 
midairJumpLeft = true; 
}
if(this.character.x + this.character.w < this.platforms[index].x + this.platforms[index].w  && this.character.x + this.character.w  - 10 >= this.platforms[index].x && this.character.y + this.character.w - 1 > this.platforms[index].y){
  playingGame = false;
  music.currentTime = 0;
  music.pause();
  if(score > hiScore){
    var playerName = window.prompt('You got a high score! Enter a nickname to display, hitting cancel will result as you showing up as anonymous');
    checkForOffensive();
  setHighScore(score); 
  getHighScore();
  updateName();
...
window.addEventListener('keydown', function(e){ 
  ...
  if(playingGame === true && gameWorld.character.x > 0){  gameWorld.character.x--;
   gameWorld.character.x--;
    gameWorld.character.x--;
     gameWorld.character.x--;
      gameWorld.character.x--;
       gameWorld.character.x--;
...
window.addEventListener('keydown', function(e){ 
  ...
  if(playingGame === true && gameWorld.character.x  + gameWorld.character.w < canvas.width){  gameWorld.character.x++;
   gameWorld.character.x++;
    gameWorld.character.x++;
     gameWorld.character.x++;
      gameWorld.character.x++;
